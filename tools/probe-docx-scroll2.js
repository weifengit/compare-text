...
var FIX = path.join(ROOT, '.probe-out3', 'fix');
...
(async function main() {
  ...
  fs.writeFileSync(path.join(FIX, 'b.docx'), await buildDocx(paras));
  ...
  var profile = path.join(os.tmpdir(), 'edge-probe3-' + process.pid);
...
function cleanup() {
  ...
  try { fs.rmSync(path.join(os.tmpdir(), 'edge-probe3-' + process.pid), { recursive: true, force: true }); } catch (e5) {}
}
